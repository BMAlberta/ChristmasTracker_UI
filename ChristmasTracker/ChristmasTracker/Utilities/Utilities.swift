...
struct ImageUtiliy {
    ...
    static func fetchImagePathForItem(urlString: String) async -> String? {
        ...
        if let imagePath = metadata.value(forKeyPath: "imageMetadata.URL") as? URL {
            return imagePath.absoluteString
        }
        
        if let iconPath = metadata.value(forKeyPath: "iconMetadata.URL") as? URL {
            return iconPath.absoluteString
        }
         return nil
    }
